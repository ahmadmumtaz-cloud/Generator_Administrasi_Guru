            ...
            <div key={item.id} className="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow fade-in">
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center space-x-2">
                  <span className={`px-2 py-1 text-xs font-semibold rounded-full ${item.module_type === 'admin' ? 'bg-blue-100 text-blue-800' : 'bg-green-100 text-green-800'}`}>
                    {item.module_type === 'admin' ? 'Administrasi' : 'Bank Soal'}
                  </span>
                  <span className="text-sm font-medium">
                    {item.mata_pelajaran} - Kelas {item.kelas} (Semester {item.semester})
                  </span>
                </div>
                    ...
                    <>
                      ...
                      <div className="text-xs text-gray-500 flex items-center flex-wrap gap-x-4 gap-y-1 pt-1">
                        ...
                        {item.sertakan_soal_tka && item.jumlah_soal_tka && (
                            <span className="font-semibold text-purple-600">[+{item.jumlah_soal_tka} Soal TKA-{item.kelompok_tka?.toUpperCase()}]</span>
                        )}
                        <span><strong>Bahasa:</strong> {item.bahasa}</span>
                        {item.jenis_soal && <span><strong>Jenis:</strong> {item.jenis_soal.join(', ')}</span>}
